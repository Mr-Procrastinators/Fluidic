...
export default function ControlPanel() {
  ...
  const setPump = (val: number) => set(ref(database, "SCADA_DATA/pump"), val);
  const setValve = (val: number) => set(ref(database, "SCADA_DATA/valve"), val);
  const setMode = (val: string) => set(ref(database, "SCADA_DATA/mode"), val);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl shadow-sm border border-slate-700 p-4" style={{ backgroundColor: "#1e293b" }}>
          ...
          <select
            value={scada.mode || "AUTO"}
            onChange={(e) => setMode(e.target.value)}
            className="mt-1 bg-blue-600 text-white font-bold text-sm rounded-lg px-3 py-1.5 border-0 outline-none cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="p-6 text-center">
            ...
            <div className={`text-2xl font-black mb-1 ${scada.pump === 1 ? "text-emerald-500" : "text-red-500"}`}>
              {scada.pump === 1 ? "RUNNING" : "STOPPED"}
            </div>
            {scada.pump === 1 && (
              <div className="w-3 h-3 mx-auto rounded-full bg-emerald-400 animate-ping mb-3" />
            )}
            <div className="flex gap-3 justify-center mt-4">
              <button onClick={() => setPump(1)} className="bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[10px] uppercase px-6 py-2.5 rounded-lg transition-colors">
                Start Pump
              </button>
              <button onClick={() => setPump(0)} className="bg-red-600 hover:bg-red-700 text-white font-black text-[10px] uppercase px-6 py-2.5 rounded-lg transition-colors">
                Stop Pump
              </button>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="p-6 text-center">
            ...
            <div className={`text-2xl font-black mb-4 ${scada.valve === 1 ? "text-blue-500" : "text-slate-500"}`}>
              {scada.valve === 1 ? "OPEN" : "CLOSED"}
            </div>
            <div className="flex gap-3 justify-center">
              <button onClick={() => setValve(1)} className="bg-blue-600 hover:bg-blue-700 text-white font-black text-[10px] uppercase px-6 py-2.5 rounded-lg transition-colors">
                Open Valve
              </button>
              <button onClick={() => setValve(0)} className="text-white font-black text-[10px] uppercase px-6 py-2.5 rounded-lg transition-colors" style={{ backgroundColor: "#334155" }}>
                Shut Valve
              </button>
            </div>
          </div>
